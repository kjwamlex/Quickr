...
- (void)commonInit {
    ...
    _hourHandWidth = 4;
    _hourHandLength = 60;
    _hourHandOffsideLength = -4;
    
    _minuteHandColor = [UIColor whiteColor];
    _minuteHandAlpha = 1.0;
    _minuteHandWidth = 3;
    _minuteHandLength = 100;
    _minuteHandOffsideLength = -4;
    
    _secondHandColor = [UIColor orangeColor];
    _secondHandAlpha = 1.0;
    _secondHandWidth = 1;
    _secondHandLength = 105;
    _secondHandOffsideLength = 5;

    _hubColor = [UIColor whiteColor];
    _hubAlpha = 1.0;
    _hubRadius = 5.0;
    
    _otherhubColor = [UIColor orangeColor];
    _otherhubalpha = 1.0;
    _otherhubradius = 3.0;
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.enableotherhub == YES) {
        CGContextSetFillColorWithColor(ctx, self.otherhubColor.CGColor);
        CGContextSetAlpha(ctx, self.otherhubalpha);
        CGPoint center = CGPointMake(self.frame.size.width / 2.0f, self.frame.size.height / 2.0f);
        CGContextAddArc(ctx, center.x, center.y, self.otherhubradius, 0, 2 * M_PI, 0);
        CGContextFillPath(ctx);
